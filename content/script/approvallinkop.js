        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var x = 0; x < data.length; x++) {
                    if (data[x].relation == 0 && x == data[x].senderid) {
        ...
        $(document).ready(function () {
            $("#frndsrch").click(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        for (var x = 0; x < data.length; x++) {
                            if (data[x].relation == 0 && x == data[x].receiverid) {
                                console.log("ID: " + data[x].loginid + " name: " + data[x].firstname + " picture: " + data[x].profilepic);
                                var html = '<img src="' + data[x].profilepic + '"style="height:55px;width:30px;"/>' + '<label style="margin-top:-35px; margin-left:50px;">' + data[x].firstname + data[x].surname + '</label> <input type="button" value="Friend" id="friendbtn" style="margin-left:135px;margin-top:-35px;" data-id="' + data[x].loginid + '"/><input type=hidden style="margin-left:210px;margin-top:-35px; height:25px;" value="' + name1 + '"/><br><hr>';
